...
| PVS | 同步间歇指令通气 | SIMV | 同步触发 | 定容/定压 | Vt, f, I:E, 触发窗 | IMV改良，触发窗**提前** |
...
**SIMV（同步间歇指令通气）**：IMV 的改良版，在指令通气前设置一个触发窗（trigger window），当患者自主吸气落在触发窗内时，通气机会与患者呼吸同步送气。当患者自主呼吸频率增加时，触发窗在时间轴上**提前**。优点：减少人机对抗，患者舒适度更好。临床上广泛用于呼吸机撤离过程中的过渡通气。
